...
#define CS_LOW()  LATBbits.LATB14 = 0	/* MMC CS = L  select */
#define CS_HIGH() LATBbits.LATB14 = 1	/* MMC CS = H  disselect*/
//#define INS	!(PORTB & (1<<11))	/* Card detected   (yes:true, no:false, default:true) */
#define INS	1	                    /* Card detected   (yes:true, no:false, default:true) */
...
// -------------------------------------------------------------------------
// add for pic32mx----------------------------------------------------------
#define SPI_BUSY SPI2STATbits.SPIBUSY
#define SPI_CHN   2
...
void init_spi (void)		/* Initialize SPI port */
{
	...
	RPB11R = 0b0100;	//RPB11 = SDO	SPI2 DATA OUTPUT

	//SPI_CHN	:チャンネル2
	//SPI_OPEN	:ビット幅8BIT、フレームなし、マスタ、クロックタイミングMODE0
	//n			:Source Clock divisor
	SpiChnOpen(SPI_CHN, SPI_OPEN, 2);	// divide fpb by 2, configure the I/O ports. Not using SS in this example
...
{
	SPI2BUF = (unsigned long)dat;
	while (SPI_BUSY) ;
//	debug_001 = (BYTE)SPI2BUF; // for debug
	return (BYTE)SPI2BUF;
}

/* Alternative macro to transfer data fast */
#define XMIT_SPI_MULTI(src,cnt) {UINT c=cnt/2; const BYTE *p=src; do {SPI2BUF=*p++; while(SPI_BUSY); SPI2BUF; SPI2BUF=*p++; while(SPI_BUSY); SPI2BUF; } while(--c);}
#define RCVR_SPI_MULTI(dst,cnt)	{UINT c=cnt/2; BYTE *p=dst; do {SPI2BUF=0xFF; while(SPI_BUSY); *p++=SPI2BUF; SPI2BUF=0xFF; while(SPI_BUSY); *p++=SPI2BUF; } while(--c);}
...
/*-----------------------------------------------------------------------*/
/* This function must be called by timer interrupt in period of 10ms      */
...
void disk_timerproc (void)
{
	...
	n = Timer1;					/* 100Hz decrement timer with zero stopped */
	if (n) Timer1 = --n;
	n = Timer2;
	if (n) Timer2 = --n;
