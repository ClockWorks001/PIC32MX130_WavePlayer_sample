...
//static volatile
//UINT uiTMR002, uiTMR003;		/* 1000Hz decrement timer */
...
{
	do {
		SPI2BUF = *buff++; while(SPI2STATbits.SPIBUSY == TRUE) ; SPI2BUF; 
		SPI2BUF = *buff++; while(SPI2STATbits.SPIBUSY == TRUE) ; SPI2BUF; 
	} while (cnt -= 2);
}
...
{
	do {
		SPI2BUF = 0xFF; while(SPI2STATbits.SPIBUSY == TRUE) ; *buff++ = SPI2BUF; 
		SPI2BUF = 0xFF; while(SPI2STATbits.SPIBUSY == TRUE) ; *buff++ = SPI2BUF; 
	} while (cnt -= 2);
}
...
int wait_ready (void)
{
	...
	uiTMR003 = 500;	/* Wait for ready in timeout of 500ms */
	do
		d = xchg_spi(0xFF);
	while ((d != 0xFF) && uiTMR003);
...
)
{
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		uiTMR002 = 1000;						/* Initialization timeout of 1000 msec */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);			/* Get trailing return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* The card can work at vdd range of 2.7-3.6V */
				while (uiTMR002 && send_cmd(ACMD41, 0x40000000));	/* Wait for leaving idle state (ACMD41 with HCS bit) */
				if (uiTMR002 && send_cmd(CMD58, 0) == 0) {			/* Check CCS bit in the OCR */
			...
			while (uiTMR002 && send_cmd(cmd, 0));		/* Wait for leaving idle state */
			if (!uiTMR002 || send_cmd(CMD16, 512) != 0)	/* Set read/write block length to 512 */
				ty = 0;
		}
	}
	CardType = ty;
...
void disk_timerproc (void)
{
    ...
    // => see the vTMR1_lib.c 20160112  
	n = uiTMR002;					// 1000Hz decrement timer with zero stopped 
	if (n) uiTMR002 = --n;
	n = uiTMR003;
	if (n) uiTMR003 = --n;


	// Update socket status => this project don't check WP & CD 20160112  
